        ...
        rfo(j, m-1, 0) {
            if(g[i][j] == '*' || g[i][j] == '+') {
                ...
                while(1) {                    
                    ...
                    bool checkIfInTable = true;
                    if(i-1-d < 0 || j+1+d >= m || j-1-d < 0) checkIfInTable = false;
                    debug(checkIfInTable);
                    if(!checkIfInTable) break;

                    bool checkIfMarked = false;
                    if((g[i-1-d][j-1-d] == '*' || g[i-1-d][j-1-d] == '+') && (g[i-1-d][j+1+d] == '*' || g[i-1-d][j+1+d] == '+')) checkIfMarked = true;
                    debug(checkIfMarked);
                    if(!checkIfMarked) break;
